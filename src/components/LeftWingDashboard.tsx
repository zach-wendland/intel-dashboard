// Left-Wing Dashboard (mirror of right-wing with progressive sources)
...
function calculateFeedMetrics(feedStatus: Record<string | number, FeedStatus>) {
  ...
  const totalCount = LEFT_LIVE_FEEDS.length;
        ...
        <div>
          ...
          <ul className="text-red-200 space-y-1 text-xs">
            {Object.entries(errorMessages).map(([feedId, error]) => {
              const feed = LEFT_LIVE_FEEDS.find(f => f.id === feedId);
              return (
                <li key={feedId}>
...
interface LeftWingDashboardProps {
  onSwitchPerspective: () => void;
}

export default function LeftWingDashboard({ onSwitchPerspective }: LeftWingDashboardProps) {
  ...
  const fetchLiveFeed = async () => {
    ...
    const promises = LEFT_LIVE_FEEDS.map(async (source) => {
  ...
  const filteredSources = filter === 'ALL' ? LEFT_SOURCES : LEFT_SOURCES.filter(s => s.category === filter);
      ...
      <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-white">LEFT-WING INTELLIGENCE GRID</h1>
          ...
          <nav className="flex gap-1 bg-slate-800/50 p-1 rounded-lg">
            ...
            <button onClick={() => setActiveTab('feed')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'feed' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>
              <Users className="h-4 w-4 inline mr-2" />Live Wire
            </button>
          ...
          <div className="flex items-center gap-4">
            <button onClick={onSwitchPerspective} className="text-xs px-3 py-1.5 bg-red-900/30 border border-red-700/50 text-red-300 rounded hover:bg-red-900/50 transition-all">
        ...
        {activeTab === 'grid' && (
          <div className="space-y-6">
            <div className="flex flex-wrap gap-2 pb-4 border-b border-slate-800">
              ...
              {Object.entries(LEFT_SOURCE_CATEGORIES).map(([key, config]) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredSources.map((source) => {
                const categoryConfig = LEFT_SOURCE_CATEGORIES[source.category];
                ...
                return (
                  <a key={source.id} href={source.url} target="_blank" rel="noopener noreferrer" className="group bg-slate-900 border border-slate-800 rounded-lg p-4 hover:border-slate-500 transition-all hover:shadow-lg hover:shadow-slate-900/50 hover:-translate-y-1 block relative">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${source.tier === 'High' ? 'bg-blue-900/20 text-blue-400 border-blue-900/30' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500" style={{ width: `${item.velocity}%` }}></div>
                        </div>
                      </div>
                    </div>
                  </a>
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
                  ...
                  Priority Issues (Simulated)
                </h3>
                <div className="space-y-3">
                  {['Climate Crisis', 'Healthcare Access', 'Income Inequality', 'Voting Rights'].map((issue, i) => (
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap gap-2">
                    {['Climate Justice', 'Labor Rights', 'Healthcare For All', 'Voting Rights', 'Wealth Tax', 'Green New Deal', 'Student Debt'].map((tag) => (
        ...
        {activeTab === 'synthesis' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={LEFT_CHART_DATA}>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={LEFT_NARRATIVE_DATA} layout="vertical">
